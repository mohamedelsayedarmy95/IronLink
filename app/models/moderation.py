...
class ContentReport(Base):
    ...
    __table_args__ = (
        ...
        Index(
            ...
            "message_id",
            unique=True,
            # `text(...)`, the SQL construct — not `Text(...)`, the column
            # type. The wrong one was here and never raised, because nothing
            # ever built this schema from the models: the index was created by
            # a hand-written migration, which used the right one. So the
            # constraint is live in the database and the model was simply an
            # unfaithful description of it — invisible until the first
            # `create_all`, and it would have produced a wrong autogenerated
            # migration long before anybody noticed.
